...
class SplitGNN_3_4(nn.Module):
    def __init__(self, hist_len, pred_len, in_dim, city_num, batch_size, device, edge_index, edge_attr, wind_mean, wind_std, node_gru_hidden_dim, edge_gru_hidden_dim, edge_mlp_hidden_dim):
        super(SplitGNN_3_4, self).__init__()
        ...
        self.edge_gru = GRUCell(23, self.edge_gru_hidden_dim)
    ...
    def generate_edge_attributes(self, wind, non_wind):
        ...
        non_wind_src = non_wind[:, edge_src]
        non_wind_target = non_wind[:, edge_target]

        return torch.cat([non_wind_src, non_wind_target, edge_atts], dim=-1)
    ...
    def forward(self, pm25_hist, feature):
        ...
        for i in range(self.pred_len):
            ...
            current_wind = x[:,:, -2:]
            non_wind = x[:,:, :-2]
            edge_atts = self.generate_edge_attributes(current_wind, non_wind)
            ...
            R = Sparsemax(dim=2, device=self.device)(R)
            R_list.append(R)
