...
class SplitGNN_2(nn.Module):
    ...
    def forward(self, pm25_hist, feature, return_R=False):
        ...
        # hn = h0
        c0 = torch.zeros(self.batch_size, self.city_num, 1).to(self.device)
        c = c0
        ...
        for i in range(self.pred_len):
            ...
            # xn = hn.view(self.batch_size, self.city_num, self.hid_dim) # TODO this probably learns interactions...
            # xn = self.fc_out(xn)
            c = self.node_mlp(torch.cat([x, c], dim=-1))
