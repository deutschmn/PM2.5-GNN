...
class SplitGNN_6(nn.Module):
    def __init__(self, hist_len, pred_len, in_dim, city_num, batch_size, device, edge_index, edge_attr, wind_mean, wind_std, node_gru_hidden_dim, edge_mlp_hidden_dim, transfer_lag):
        super(SplitGNN_6, self).__init__()
        ...
        # in: in features + PM2.5 from prev. iteration
        self.node_mlp = Sequential(Linear(self.in_dim + 1, self.hid_dim),
                                   nn.ReLU(),
                                   Linear(self.hid_dim, 1))
        ...
        # transfers
        self.transfer_lag = transfer_lag 
        self.R_mlp = Linear(self.transfer_lag, self.transfer_lag)
    ...
    def restructure_edges(self, e_rep):
        ...
        R_2d = torch.zeros(self.batch_size, self.num_nodes * self.num_nodes, device=self.device)
        R_2d = R_2d.scatter_(1, self.edge_restructure_idx, e_rep).view(self.batch_size, self.num_nodes, self.num_nodes)
        return R_2d
    ...
    def forward(self, pm25_hist, feature):
        ...
        R = torch.zeros(self.batch_size, self.num_nodes, self.num_nodes, self.transfer_lag).to(self.device)
        c = torch.zeros(self.batch_size, self.num_nodes, self.transfer_lag).to(self.device)
        ...
        for i in range(self.pred_len):
            ...
            R_2d = self.restructure_edges(en_rep)
            R_2d = torch.softmax(R_2d, dim=2)

            R[:, :, :, 0] = R_2d
            R = self.R_mlp(R)
            R = torch.softmax(R, dim=-1)

            # TODO probably need an MLP here to adjust for time lags?

            # compute local phenomena
            node_in = torch.cat([x, cn], dim=2)
            node_out = self.node_mlp(node_in)
            c[:, :, 0] = node_out.squeeze()

            # execute transfers
            contributions = torch.zeros(self.batch_size, self.num_nodes, self.transfer_lag).to(self.device)
            for d in range(self.transfer_lag):
                cont = torch.matmul(R[:, :, :, d], c[:, :, d].unsqueeze(dim=-1))
                contributions[:, :, d] = cont.squeeze()
            cn = torch.sum(contributions, -1).unsqueeze(dim=-1)
            cn = torch.clamp(cn, 0, 300) # TODO verify values

            c[:, :, 0] = cn.squeeze()

            if i >= self.transfer_lag:
                R_list.append(R)
                pm25_pred.append(cn)
